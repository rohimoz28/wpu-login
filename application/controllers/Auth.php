...
class Auth extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        // jika usernya ada
        if ($user) {
            // cek apakah usernya sudah di aktivasi?
            if ($user['is_active'] == 1) {
                // cek apakah benar passwordnya?
                if (password_verify($password, $user['password'])) {
                    // hal pertama yang dilakukan adalah simpan session agar bisa di gunakan di halaman user yg sukses login
                    $data = [
                        'email' => $user['email'],
  ...
  Account has been registered. Please Login! </div>');
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'sinofsloth93@gmail.com',
            'smtp_pass' => 'emotic182',
            'smtp_port' => '465',
            'mailtype' => 'html',
            'charset' => 'utf-8',
        ...
        $this->email->from('sinofsloth93@gmail.com', 'Capt.Meliodas');
  ...
  ' . $email . ' has been activated. Please login!</div>');
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
  Activated failed. Token expired!</div>');
                    redirect('auth');
                }
            } else {
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
  Activated failed. Token invalid!</div>');
                redirect('auth');
            }
        } else {
            $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
  Activated failed. Wrong email!</div>');
            redirect('auth');
        }
    }
  ...
  Email not registered</div>');
    ...
    {
        $this->form_validation->set_rules('password1', 'New password', 'min_length[3]|required|matches[password2]|trim');
        $this->form_validation->set_rules('password2', 'Repeat password', 'min_length[3]|required|matches[password1]|trim');
